...
def main():
    ...
    while True:
        ...
        if m := re.match(r'insert(?: (\d+))?(?: (\d+))? \"(.+)\"$', user_input):
            match len(args := [x for x in m.groups() if x is not None]):
                case 1:
                    document.insert_line(text=args[0])
                case 2:
                    document.insert_line(line_number=int(args[0]), text=args[1])
                case 3:
                    document.insert_line(line_number=int(args[0]), column_number=int(args[1]), text=args[2])
        elif m := re.match(r'delete (\d+)$', user_input):
            document.delete_line(line_number=int(m.group(1)))
        elif m := re.match(r'swap (\d+) (\d+)$', user_input):
            if len(args := [x for x in m.groups() if x is not None]) == 2:
                document.swap_lines(int(args[0]), int(args[1]))
