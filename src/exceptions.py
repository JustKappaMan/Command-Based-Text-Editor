...
class WrongNumberOfCommandLineArgs(Exception):
    ...
    def __init__(self, expected_args_count: int, actual_args_count: int) -> None:
        self.expected_args_count: int = expected_args_count
        self.actual_args_count: int = actual_args_count
        self.message: str = f'Error! Expected number of CL args: {expected_args_count}, but got {actual_args_count}.'
...
class PathDoesNotExist(Exception):
    ...
    def __init__(self, path: pathlib.Path) -> None:
        ...
        self.message: str = f"Error! Following path doesn't exist: {self.path}"
        super().__init__(self.message)
...
class PathIsNotFilepath(Exception):
    ...
    def __init__(self, path: pathlib.Path) -> None:
        self.path: pathlib.Path = path
        self.message: str = f"Error! Following path isn't a filepath: {self.path}"
        super().__init__(self.message)
...
class ZeroLineNumber(Exception):
    ...
    def __init__(self) -> None:
        self.message: str = "Error! You can't access the line №0. The number of the line must be greater than 0."
        super().__init__(self.message)
...
class TooLargeLineNumber(Exception):
    ...
    def __init__(self, line_number: int) -> None:
        ...
        self.message: str = f"Error! You can't access the line №{self.line_number}. " \
...
class ZeroColumnNumber(Exception):
    ...
    def __init__(self) -> None:
        self.message: str = "Error! You can't access the column №0. " \
...
class TooLargeColumnNumber(Exception):
    ...
    def __init__(self, column_number: int) -> None:
        ...
        self.message: str = f"Error! You can't access the column №{self.column_number}. " \
...
class LineSwappedWithItself(Exception):
    ...
    def __init__(self) -> None:
        self.message: str = "Error! You can't swap the line with itself."
        super().__init__(self.message)
...
class UnsavedChangesExist(Exception):
    ...
    def __init__(self) -> None:
        self.message: str = 'All unsaved changes will be lost. Are you sure you want to close the editor? (Y/n): '
        super().__init__(self.message)
